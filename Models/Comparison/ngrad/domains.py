...
# -------------------------------------------------------------------
# Rectangle domain: supports arbitrary lower/upper bounds
# -------------------------------------------------------------------
class Rectangle(Hyperrectangle):
    """
    Axis-aligned rectangle [a0, b0] x [a1, b1].

    Parameters
    ----------
    intervals : array-like, shape (2, 2)
        intervals = ((a0, b0), (a1, b1))
        e.g. intervals = ((0., 2*pi), (0., T))
    """

    def __init__(self, intervals):
        # intervals should be [[a0,b0], [a1,b1]]
        super().__init__(intervals)

    def deterministic_integration_points(self, N: int):
        """
        Grid-based integration points on [a0,b0] x [a1,b1].

        N is a density parameter: the number of points in each
        direction is roughly ~(b-a)*N (at least 1).
        """
        a0 = self._l_bounds[0]
        b0 = self._r_bounds[0]
        a1 = self._l_bounds[1]
        ...
        M0 = jnp.maximum(math.ceil((b0 - a0) * N), 1)
        M1 = jnp.maximum(math.ceil((b1 - a1) * N), 1)

        xs = jnp.linspace(a0, b0, M0)
        ts = jnp.linspace(a1, b1, M1)
        X, T_ = jnp.meshgrid(xs, ts, indexing="ij")
        pts = jnp.stack([X.reshape(-1), T_.reshape(-1)], axis=1)
        return pts
