...
Gradient descent with line search for the 2-D Helmholtz equation.

We solve on Ω=(0,1)^2:
    -Δu + k^2 u = f,   u|∂Ω=0,
with u*(x,y)=sin(πx)sin(πy), hence f=(2π²+k²)u*.

Logs loss and L2/H1 errors, and saves metrics and solution grid.
"""
...
def main(
    ...
    for iteration in tqdm(range(steps + 1), desc="GD Training", unit="it"):
        ...
        if iteration % log_every == 0:
            ...
            tqdm.write(
                f"GD Iter {iteration}: loss={l_val:.6e}, L2={l2_err:.6e}, H1={h1_err:.6e}, step={actual_step}, k={float(helmholtz_k)}"
            )
    ...
    np.savez(os.path.join(run_dir, "gd_metrics.npz"), **metrics)
    print(f"Metrics saved to {os.path.join(run_dir, 'gd_metrics.npz')}")
...
if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="Train 2D Helmholtz PINN with GD+line-search and log metrics.")
