...
class DeepONet(nn.Module):
    ...
    """
    def __init__(self, m: int, width: int = 50, depth: int = 5, feat_dim: int = 50):
        super().__init__()
        hidden = [width] * (depth - 1)
        self.branch = mlp([m] + hidden + [feat_dim], activation=nn.Tanh)
        self.trunk  = mlp([1] + hidden + [feat_dim], activation=nn.Tanh)
